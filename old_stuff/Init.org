This Configuration is heaviely inspired by the Emacs from Scratch series by David Wilson from the the Config used in the Videos: [[https://github.com/daviwil/emacs-from-scratch][emacs-from-scratch]]
...
#+begin_src emacs-lisp
  ...
  (setq pyexe (executable-find "python"))
  (setq py3exe (executable-find "python3"))

  (defun python-exe ()
    (if pyexe
        (if (= 0 (call-process pyexe nil nil nil "--version"))
            :t
          nil)
      nil))
  ...
  (defun python3-exe ()
    (if py3exe
        (if (= 0 (call-process py3exe nil nil nil "--version"))
            true
          nil)
      nil))
